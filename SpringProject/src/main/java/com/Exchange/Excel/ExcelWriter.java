...
public class ExcelWriter {
    ...
    public ExcelWriter(RatesData data) {
        this.data=data;
        try {
            File newFile = new File("CURRENCY.xlsx");
            if (!newFile.exists()) {
                OutputStream fileOut = new FileOutputStream("CURRENCY.xlsx");
                workbook =new XSSFWorkbook();
                sheet = workbook.createSheet("exchangeData");
            }else{
                if (newFile.exists()) {
                    workbook =new XSSFWorkbook(new FileInputStream("CURRENCY.xlsx"));
                    sheet=workbook.getSheetAt(0);
                }
            }

            outFile = new FileOutputStream("CURRENCY.xlsx");
